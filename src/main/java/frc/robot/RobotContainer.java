...
public class RobotContainer {
  ...
  private ChassisSpeeds blank;
  private ChassisSpeeds constX;
  ...
  private void configureBindings() {
    ...
    // m_XboxDriver.povRight().whileTrue(Commands.run(() -> drivetrain.applyRequest(() -> drive.withRotationalRate(m_Vision.turnToNote()))));
    // Subsystems
    ...
    m_XboxDriver.leftStick().and(() -> !noteHandle.hasNote()).whileTrue(drivetrain.applyRequest(() -> m_swerveRequest.withCenterOfRotation(DriveConstants.Center).withSpeeds(noteTrack)));
    ...
    NamedCommands.registerCommand("AutoNote", AutonTrack());
  ...
  public Command AutonTrack() {
    if (!m_Vision.cameraHasNote()) {
      return (drivetrain
      .applyRequest(() -> m_swerveRequest.withCenterOfRotation(DriveConstants.Center)
      .withSpeeds(constX)))
      .unless(() -> noteHandle.hasNote());
    } else {
      return (drivetrain
        .applyRequest(() -> m_swerveRequest.withCenterOfRotation(DriveConstants.Center)
        .withSpeeds(noteTrackAuto)))
        .unless(() -> noteHandle.hasNote());
    }
  }
  ...
  public void updateValues() {
    ...
    noteTrackAuto = new ChassisSpeeds(
        ...
        0,
        m_Vision.turnToNote());
    blank = new ChassisSpeeds(
        0,
        0,
        0);
    constX = new ChassisSpeeds(
        1,
        0,
        0);
